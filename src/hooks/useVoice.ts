...
function hasTranscriptionBackend(): boolean {
  return (
    isVoiceStreamAvailable() || isOpenAICompatibleTranscriptionAvailable()
  )
}
...
export function useVoice({
  ...
  function finishRecording(): void {
    ...
    const usingVoiceStream = isVoiceStreamAvailable()
    const recordedAudio = fullAudioRef.current.slice()
    ...
    void finalizePromise
      .then(async finalizeSource => {
        if (isStale()) return
        if (!usingVoiceStream && recordedAudio.length > 0) {
          try {
            const stt = normalizeLanguageForSTT(getInitialSettings().language)
            const text = await transcribeOpenAICompatibleSpeech({
              pcmChunks: recordedAudio,
              language: stt.code,
            })
            if (isStale()) return
            ...
            onErrorRef.current?.(
              'Voice transcription failed. Check OPENAI_SPEECH_* settings and your network connection.',
            )
            fullAudioRef.current = []
            accumulatedRef.current = ''
            updateState('idle')
        ...
        if (text) {
          ...
          if (usingVoiceStream && !wsConnected) {
  ...
  async function startRecordingSession(): Promise<void> {
    ...
    if (!isVoiceStreamAvailable() && isOpenAICompatibleTranscriptionAvailable()) {
      logForDebugging(
        '[voice] Using OpenAI-compatible transcription backend (non-streaming)',
      )
      return
    }
  ...
  const handleKeyEvent = useCallback(
    (fallbackMs = REPEAT_FALLBACK_MS): void => {
      if (!enabled || !hasTranscriptionBackend()) {
        return
      }
