...
import { shouldUseChineseUi } from './uiLanguage.js';
export interface PreflightCheckResult {
  success: boolean;
  error?: string;
  sslHint?: string;
}
async function checkEndpoints(useChineseUi: boolean): Promise<PreflightCheckResult> {
  try {
    ...
    const checkEndpoint = async (url: string): Promise<PreflightCheckResult> => {
      try {
        ...
        if (response.status !== 200) {
          ...
          return {
            success: false,
            error: useChineseUi ? `无法连接到 ${hostname}：状态码 ${response.status}` : `Failed to connect to ${hostname}: Status ${response.status}`
          };
        }
        return {
          success: true
        ...
        return {
          success: false,
          error: useChineseUi ? `无法连接到 ${hostname}：${error instanceof Error ? (error as ErrnoException).code || error.message : String(error)}` : `Failed to connect to ${hostname}: ${error instanceof Error ? (error as ErrnoException).code || error.message : String(error)}`,
    ...
    return {
      success: false,
      error: useChineseUi ? `连接检查错误：${error instanceof Error ? (error as ErrnoException).code || error.message : String(error)}` : `Connectivity check error: ${error instanceof Error ? (error as ErrnoException).code || error.message : String(error)}`
    };
  }
}
...
export function PreflightStep(t0) {
  ...
  if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
    t1 = () => {
      const run = async function run() {
        const checkResult = await checkEndpoints(useChineseUi);
  ...
  if ($[6] !== isChecking || $[7] !== result || $[8] !== showSpinner) {
    t5 = isChecking && showSpinner ? <Box paddingLeft={1}><Spinner /><Text>{useChineseUi ? '正在检查网络连接…' : 'Checking connectivity...'}</Text></Box> : !result?.success && !isChecking && <Box flexDirection="column" gap={1}><Text color="error">{useChineseUi ? '无法连接到 Anthropic 服务' : 'Unable to connect to Anthropic services'}</Text><Text color="error">{result?.error}</Text>{result?.sslHint ? <Box flexDirection="column" gap={1}><Text>{result.sslHint}</Text><Text color="suggestion">{useChineseUi ? '请参见 https://code.claude.com/docs/en/network-config' : 'See https://code.claude.com/docs/en/network-config'}</Text></Box> : <Box flexDirection="column" gap={1}><Text>{useChineseUi ? '请检查你的网络连接和网络设置。' : 'Please check your internet connection and network settings.'}</Text><Text>{useChineseUi ? '注意：Claude Code 可能在你所在国家/地区不可用。支持国家列表见 ' : 'Note: Claude Code might not be available in your country. Check supported countries at '}{' '}<Text color="suggestion">https://anthropic.com/supported-countries</Text></Text></Box>}</Box>;
    $[6] = isChecking;
    $[7] = result;
    $[8] = showSpinner;
    $[9] = t5;
