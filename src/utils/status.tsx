...
import { getAccountInformation, isClaudeAISubscriber, isCodexSubscriber } from './auth.js';
...
} from './model/externalProvider.js';
import {
  getOpenAICompatibleSpeechBaseUrl,
  getOpenAICompatibleSpeechSttModel,
  getOpenAICompatibleSpeechTtsModel,
  getOpenAICompatibleSpeechTtsVoice,
  isOpenAICompatibleSpeechEnabled,
...
export function buildAPIProviderProperties(): Property[] {
  const usingCodex = isCodexSubscriber();
  const externalProvider = getExternalModelProvider();
  const apiProvider = getAPIProvider();
  const speechEnabled = isOpenAICompatibleSpeechEnabled();
  const properties: Property[] = [];
  if (usingCodex) {
    properties.push({
      label: 'API provider',
    ...
    properties.push({
      label: 'Default model',
      value: process.env.OPENAI_MODEL?.trim() || 'gpt-5.3-codex'
    });
  } else if (externalProvider) {
    const providerLabel = externalProvider === "openai" ? "OpenAI API" : "Ollama (local)";
    properties.push({
      label: 'API provider',
      value: providerLabel
    });
    properties.push({
      label: 'Endpoint',
      value: getExternalProviderBaseUrl(externalProvider)
    });
    properties.push({
      label: 'Default model',
      value: getExternalProviderDefaultModel(externalProvider)
    });
    if (externalProvider === "openai") {
      properties.push({
        label: 'API key',
        value: process.env.OPENAI_API_KEY ? 'OPENAI_API_KEY' : 'missing'
      });
    }
  ...
  if (speechEnabled) {
    properties.push({
      ...
      value: getOpenAICompatibleSpeechBaseUrl()
    });
    if (isOpenAICompatibleTranscriptionAvailable()) {
      properties.push({
        label: 'Speech input',
        value: `OpenAI-compatible STT (${getOpenAICompatibleSpeechSttModel()})`
      });
    }
    if (isOpenAICompatibleTtsAvailable()) {
      properties.push({
        label: 'Speech output',
        value: `OpenAI-compatible TTS (${getOpenAICompatibleSpeechTtsModel()}, voice ${getOpenAICompatibleSpeechTtsVoice()})`
      });
    }
  }
  if (!usingCodex && !externalProvider && apiProvider === 'firstParty') {
  ...
  } else if (!usingCodex && !externalProvider && apiProvider === 'bedrock') {
  ...
  } else if (!usingCodex && !externalProvider && apiProvider === 'vertex') {
  ...
  } else if (!usingCodex && !externalProvider && apiProvider === 'foundry') {
...
export function getModelDisplayLabel(mainLoopModel: string | null): string {
  ...
  if (mainLoopModel === null && (isClaudeAISubscriber() || isCodexSubscriber())) {
