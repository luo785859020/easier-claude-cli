  ...
  async call({ setting, value }: Input, context): Promise<{ data: Output }> {
    ...
    if (
      ...
      if (!isVoiceModeEnabled()) {
        const { isOpenAICompatibleTranscriptionAvailable } = await import(
          '../../services/openaiCompatibleSpeech.js'
        )
        return {
          data: {
            success: false,
            error: isOpenAICompatibleTranscriptionAvailable()
              ? 'Voice mode is not available.'
              : 'Voice mode requires either a Claude.ai login or OPENAI_SPEECH_API_KEY / OPENAI_SPEECH_BASE_URL / OPENAI_SPEECH_STT_MODEL.',
          },
        }
      }
    ...
    if (
      feature('VOICE_MODE') &&
      setting === 'speechOutputEnabled' &&
      finalValue === true
    ) {
      const { isOpenAICompatibleTtsAvailable } = await import(
        '../../services/openaiCompatibleSpeech.js'
      )
      const { isAudioPlaybackAvailable } = await import(
        '../../services/audioPlayback.js'
      )
      ...
      if (!isOpenAICompatibleTtsAvailable()) {
        return {
          data: {
            success: false,
            error:
              'Speech output requires OPENAI_SPEECH_API_KEY / OPENAI_SPEECH_BASE_URL / OPENAI_SPEECH_TTS_MODEL.',
          },
        }
      }

      if (!isAudioPlaybackAvailable()) {
        return {
          data: {
            success: false,
            error:
              'Speech output could not find a local audio playback command.',
          },
        }
      }
    }
