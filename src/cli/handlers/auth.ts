...
import {
  runExternalProviderCompletion,
  verifyExternalProviderLogin,
} from '../../services/api/externalProvider.js'
import {
...
export async function authLogin({
  ...
  if (externalProvider) {
    const model = getExternalProviderDefaultModel(externalProvider)
    const result = await verifyExternalProviderLogin(externalProvider, {
      model,
    })
    const providerName = externalProvider === 'openai' ? 'OpenAI' : 'Ollama'
    if (!result.ok) {
      process.stderr.write(`Login failed: ${result.message}\n`)
      if (externalProvider === 'openai') {
        process.stderr.write(
          'Tip: set OPENAI_API_KEY and optionally OPENAI_MODEL / OPENAI_BASE_URL, or save them via /login.\n',
        )
      } else {
        process.stderr.write(
          'Tip: ensure Ollama is running and set OLLAMA_BASE_URL / OLLAMA_MODEL if needed, or save them via /login.\n',
        )
      }
      process.exit(1)
    }
    try {
      ...
      process.stderr.write(
        `${providerName} connected, but model warm-up failed for "${model}": ${msg}\n`,
      )
      process.exit(1)
    }
    process.stdout.write(
      `${providerName} login successful. Default model: ${model}\n`,
    )
    process.exit(0)
  }
...
export async function authStatus(opts: {
  ...
  const codexModel = process.env.OPENAI_MODEL?.trim() || 'gpt-5.3-codex'
  const hasOpenAIKey = !!process.env.OPENAI_API_KEY
  const loggedIn = usingCodex
    ? hasCodexToken
    : externalProvider
  ...
  } else if (externalProvider === 'openai') {
    authMethod = 'openai_api_key'
  } else if (externalProvider === 'ollama') {
  ...
  if (opts.text) {
    ...
    if (externalProvider === 'openai' && !hasOpenAIKey) {
      process.stdout.write(
        'Not logged in. Set OPENAI_API_KEY (and optionally OPENAI_MODEL / OPENAI_BASE_URL), or configure them in /login.\n',
      )
    }
    if (!loggedIn) {
      process.stdout.write(
    ...
    const apiProvider = usingCodex ? 'codex' : getAPIProvider()
    ...
    if (usingCodex) {
      output.externalProvider = 'codex'
      output.externalDefaultModel = codexModel
    }
    if (externalProvider) {
      output.externalProvider = externalProvider
      output.externalDefaultModel = getExternalProviderDefaultModel(
        externalProvider,
      )
    }
