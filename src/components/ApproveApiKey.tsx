...
export function ApproveApiKey(t0) {
  ...
  if ($[8] === Symbol.for("react.memo_cache_sentinel")) {
    t5 = <Text>{useChineseUi ? '是否使用这个 API Key？' : 'Do you want to use this API key?'}</Text>;
    $[8] = t5;
  } else {
    t5 = $[8];
  }
  ...
  if ($[10] === Symbol.for("react.memo_cache_sentinel")) {
    t7 = [t6, {
      label: <Text>{useChineseUi ? '否（' : 'No ('}<Text bold={true}>{useChineseUi ? '推荐' : 'recommended'}</Text>{useChineseUi ? '）' : ')'}</Text>,
      value: "no"
    }];
    $[10] = t7;
  } else {
  ...
  if ($[13] !== t2 || $[14] !== t4 || $[15] !== t8) {
    t9 = <Dialog title={useChineseUi ? '检测到环境变量中的自定义 API Key' : 'Detected a custom API key in your environment'} color="warning" onCancel={t2}>{t4}{t5}{t8}</Dialog>;
    $[13] = t2;
    $[14] = t4;
    $[15] = t8;
    $[16] = t9;
