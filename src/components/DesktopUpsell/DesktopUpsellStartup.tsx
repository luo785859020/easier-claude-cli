...
export function DesktopUpsellStartup(t0) {
  ...
  if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
    t3 = {
      label: useChineseUi ? '在 Claude Code Desktop 中打开' : 'Open in Claude Code Desktop',
      value: "try" as const
    };
    $[5] = t3;
  } else {
  ...
  if ($[7] === Symbol.for("react.memo_cache_sentinel")) {
    t5 = [t3, t4, {
      label: useChineseUi ? '不再提示' : "Don't ask again",
  ...
  if ($[8] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = <Box marginBottom={1}><Text>{useChineseUi ? '同样的 Claude Code，支持可视化 diff、实时应用预览、并行会话等能力。' : 'Same Claude Code with visual diffs, live app preview, parallel sessions, and more.'}</Text></Box>;
    $[8] = t6;
  } else {
    t6 = $[8];
  }
  ...
  if ($[11] !== handleSelect || $[12] !== t7) {
    t8 = <PermissionDialog title={useChineseUi ? '试试 Claude Code Desktop' : 'Try Claude Code Desktop'}><Box flexDirection="column" paddingX={2} paddingY={1}>{t6}<Select options={options} onChange={handleSelect} onCancel={t7} /></Box></PermissionDialog>;
    $[11] = handleSelect;
    $[12] = t7;
    $[13] = t8;
  } else {
