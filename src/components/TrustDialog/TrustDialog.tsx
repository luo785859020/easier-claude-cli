...
export function TrustDialog(t0) {
  ...
  if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
    ...
    t17 = <Text>{useChineseUi ? '快速安全检查：这个项目是你创建的，或者是你信任的吗？（比如你自己的代码、知名开源项目，或来自你团队的项目）。如果不是，请先花一点时间检查这个文件夹里的内容。' : 'Quick safety check: Is this a project you created or one you trust? (Like your own code, a well-known open source project, or work from your team). If not, take a moment to review what is in this folder first.'}</Text>;
    t18 = <Text>{useChineseUi ? 'Claude Code 将能够在这里读取、编辑并执行文件。' : 'Claude Code will be able to read, edit, and execute files here.'}</Text>;
    $[20] = t16;
    $[21] = t17;
    $[22] = t18;
  } else {
  ...
  if ($[23] === Symbol.for("react.memo_cache_sentinel")) {
    t19 = <Text dimColor={true}><Link url="https://code.claude.com/docs/en/security">{useChineseUi ? '安全指南' : 'Security guide'}</Link></Text>;
    $[23] = t19;
  } else {
    t19 = $[23];
  }
  ...
  if ($[24] === Symbol.for("react.memo_cache_sentinel")) {
    t20 = [{
      label: useChineseUi ? '是，我信任这个文件夹' : 'Yes, I trust this folder',
      value: "enable_all"
    }, {
  ...
  if ($[27] !== exitState.keyName || $[28] !== exitState.pending) {
    t22 = <Text dimColor={true}>{exitState.pending ? <>{useChineseUi ? '再次按 ' : 'Press '}{exitState.keyName}{useChineseUi ? ' 退出' : ' again to exit'}</> : <>{useChineseUi ? '回车确认 · Esc 取消' : 'Enter to confirm · Esc to cancel'}</>}</Text>;
  ...
  let t23;
  if ($[30] !== t21 || $[31] !== t22) {
    t23 = <PermissionDialog color="warning" titleColor="warning" title={useChineseUi ? '正在访问工作区：' : 'Accessing workspace:'}><Box flexDirection="column" gap={1} paddingTop={1}>{t16}{t17}{t18}{t19}{t21}{t22}</Box></PermissionDialog>;
    $[30] = t21;
    $[31] = t22;
    $[32] = t23;
  } else {
