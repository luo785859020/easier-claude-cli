...
export function ThemePicker(t0) {
  ...
  if ($[7] === Symbol.for("react.memo_cache_sentinel")) {
    t10 = [...(feature("AUTO_THEME") ? [{
      label: useChineseUi ? '自动（跟随终端）' : "Auto (match terminal)",
      value: "auto" as const
    }] : []), {
      label: useChineseUi ? '深色模式' : "Dark mode",
      value: "dark"
    }, {
      label: useChineseUi ? '浅色模式' : "Light mode",
      value: "light"
    }, {
      label: useChineseUi ? '深色模式（色盲友好）' : "Dark mode (colorblind-friendly)",
      value: "dark-daltonized"
    }, {
      label: useChineseUi ? '浅色模式（色盲友好）' : "Light mode (colorblind-friendly)",
      value: "light-daltonized"
    }, {
      label: useChineseUi ? '深色模式（仅 ANSI 颜色）' : "Dark mode (ANSI colors only)",
      value: "dark-ansi"
    }, {
      label: useChineseUi ? '浅色模式（仅 ANSI 颜色）' : "Light mode (ANSI colors only)",
      value: "light-ansi"
    }];
    $[7] = t10;
  } else {
  ...
  let t11;
  if ($[8] !== showIntroText) {
    t11 = showIntroText ? <Text>{useChineseUi ? '开始使用吧。' : "Let's get started."}</Text> : <Text bold={true} color="permission">{useChineseUi ? '主题' : 'Theme'}</Text>;
    $[8] = showIntroText;
    $[9] = t11;
  } else {
    t11 = $[9];
  ...
  if ($[10] === Symbol.for("react.memo_cache_sentinel")) {
    t12 = <Text bold={true}>{useChineseUi ? '选择最适合你终端的文本样式' : 'Choose the text style that looks best with your terminal'}</Text>;
    $[10] = t12;
  } else {
    t12 = $[10];
  }
  ...
  const t22 = colorModuleUnavailableReason === "env" ? useChineseUi ? `语法高亮已禁用（通过 CLAUDE_CODE_SYNTAX_HIGHLIGHT=${process.env.CLAUDE_CODE_SYNTAX_HIGHLIGHT}）` : `Syntax highlighting disabled (via CLAUDE_CODE_SYNTAX_HIGHLIGHT=${process.env.CLAUDE_CODE_SYNTAX_HIGHLIGHT})` : syntaxHighlightingDisabled ? useChineseUi ? `语法高亮已禁用（按 ${syntaxToggleShortcut} 启用）` : `Syntax highlighting disabled (${syntaxToggleShortcut} to enable)` : syntaxTheme ? useChineseUi ? `语法主题：${syntaxTheme.theme}${syntaxTheme.source ? `（来源：${syntaxTheme.source}）` : ""}（按 ${syntaxToggleShortcut} 关闭）` : `Syntax theme: ${syntaxTheme.theme}${syntaxTheme.source ? ` (from ${syntaxTheme.source})` : ""} (${syntaxToggleShortcut} to disable)` : useChineseUi ? `语法高亮已启用（按 ${syntaxToggleShortcut} 关闭）` : `Syntax highlighting enabled (${syntaxToggleShortcut} to disable)`;
  let t23;
  if ($[37] !== t22) {
  ...
  if (!showIntroText) {
    ...
    if ($[50] !== exitState || $[51] !== hideEscToCancel) {
      t28 = !hideEscToCancel && <Box><Text dimColor={true} italic={true}>{exitState.pending ? <>{useChineseUi ? `再按一次 ${exitState.keyName} 退出` : `Press ${exitState.keyName} again to exit`}</> : <Byline><KeyboardShortcutHint shortcut="Enter" action="select" /><KeyboardShortcutHint shortcut="Esc" action="cancel" /></Byline>}</Text></Box>;
      $[50] = exitState;
      $[51] = hideEscToCancel;
      $[52] = t28;
    } else {
