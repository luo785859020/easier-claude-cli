...
export function AutoModeOptInDialog(t0) {
  ...
  if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = true ? [{
      label: useChineseUi ? '是，并设为默认模式' : "Yes, and make it my default mode",
  ...
  if ($[6] === Symbol.for("react.memo_cache_sentinel")) {
    t5 = {
      label: useChineseUi ? '是，启用自动模式' : "Yes, enable auto mode",
  ...
  const t6 = declineExits ? useChineseUi ? '否，退出' : 'No, exit' : useChineseUi ? '否，返回' : 'No, go back';
  let t7;
  if ($[7] !== t6) {
    t7 = [...t4, t5, {
      label: t6,
  ...
  if ($[15] !== onDecline || $[16] !== t9) {
    t10 = <Dialog title={useChineseUi ? '启用自动模式？' : 'Enable auto mode?'} color="warning" onCancel={onDecline}>{t3}{t9}</Dialog>;
    $[15] = onDecline;
    $[16] = t9;
    $[17] = t10;
  } else {
