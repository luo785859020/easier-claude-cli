...
export function IdeOnboardingDialog(t0) {
  ...
  const pluginOrExtension = useChineseUi ? isJetBrains ? "插件" : "扩展" : isJetBrains ? "plugin" : "extension";
  ...
  let t6;
  if ($[8] !== ideName) {
    t6 = <>{t5}<Text>{useChineseUi ? `欢迎使用 ${ideName} 版 Claude Code` : `Welcome to Claude Code for ${ideName}`}</Text></>;
    $[8] = ideName;
    $[9] = t6;
  } else {
    t6 = $[9];
  }
  const t7 = installedVersion ? useChineseUi ? `已安装${pluginOrExtension} v${installedVersion}` : `installed ${pluginOrExtension} v${installedVersion}` : undefined;
  let t8;
  if ($[10] === Symbol.for("react.memo_cache_sentinel")) {
    t8 = <Text color="suggestion">{useChineseUi ? '⧉ 打开的文件' : '⧉ open files'}</Text>;
    $[10] = t8;
  } else {
    t8 = $[10];
  }
  ...
  if ($[11] === Symbol.for("react.memo_cache_sentinel")) {
    t9 = <Text>{useChineseUi ? '• Claude 可读取 ' : '• Claude has context of '}{t8}{" "}{useChineseUi ? '以及 ' : 'and '}<Text color="suggestion">{useChineseUi ? '⧉ 已选中的代码行' : '⧉ selected lines'}</Text></Text>;
    $[11] = t9;
  } else {
    t9 = $[11];
  }
  ...
  if ($[13] === Symbol.for("react.memo_cache_sentinel")) {
    t11 = <Text>{useChineseUi ? '• 可在 IDE 中查看 Claude Code 的改动 ' : "• Review Claude Code's changes "}{t10}{" "}<Text color="diffRemovedWord">-22</Text>{useChineseUi ? '，更直观更方便' : ' in the comfort of your IDE'}</Text>;
    $[13] = t11;
  } else {
    t11 = $[13];
  }
  ...
  if ($[14] === Symbol.for("react.memo_cache_sentinel")) {
    t12 = <Text>• Cmd+Esc<Text dimColor={true}>{useChineseUi ? ' 快速启动' : ' for Quick Launch'}</Text></Text>;
    $[14] = t12;
  } else {
    t12 = $[14];
  }
  ...
  if ($[15] === Symbol.for("react.memo_cache_sentinel")) {
    t13 = <Box flexDirection="column" gap={1}>{t9}{t11}{t12}<Text>• {mentionShortcut}<Text dimColor={true}>{useChineseUi ? ' 在输入中引用文件或代码行' : ' to reference files or lines in your input'}</Text></Text></Box>;
    $[15] = t13;
  } else {
    t13 = $[15];
  }
  ...
  if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
    t15 = <Box paddingX={1}><Text dimColor={true} italic={true}>{useChineseUi ? '按 Enter 继续' : 'Press Enter to continue'}</Text></Box>;
    $[20] = t15;
  } else {
    t15 = $[20];
  }
