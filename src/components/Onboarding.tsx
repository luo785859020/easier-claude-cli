...
export function Onboarding({
  ...
  const themeStep = <Box marginX={1}>
      <ThemePicker onThemeSelect={handleThemeSelection} showIntroText={true} helpText={useChineseUi ? "之后可通过 /theme 修改" : "To change this later, run /theme"} hideEscToCancel={true} skipExitHandling={true} // Skip exit handling as Onboarding already handles it
    />
    </Box>;
  const securityStep = <Box flexDirection="column" gap={1} paddingLeft={1}>
      <Text bold>{useChineseUi ? '安全提示：' : 'Security notes:'}</Text>
      <Box flexDirection="column" width={70}>
        ...
        <OrderedList>
          <OrderedList.Item>
            <Text>{useChineseUi ? 'Claude 可能会出错' : 'Claude can make mistakes'}</Text>
            <Text dimColor wrap="wrap">
              {useChineseUi ? '请始终审核 Claude 的回复，尤其是在' : 'You should always review Claude&apos;s responses, especially when'}
              <Newline />
              {useChineseUi ? '执行代码时。' : 'running code.'}
              <Newline />
            </Text>
          </OrderedList.Item>
          <OrderedList.Item>
            <Text>
              {useChineseUi ? '由于提示注入风险，请仅在你信任的代码上使用它' : 'Due to prompt injection risks, only use it with code you trust'}
            </Text>
            <Text dimColor wrap="wrap">
              {useChineseUi ? '更多详情请参见：' : 'For more details see:'}
              <Newline />
  ...
  if (shouldOfferTerminalSetup()) {
    steps.push({
      ...
      component: <Box flexDirection="column" gap={1} paddingLeft={1}>
          <Text bold>{useChineseUi ? '是否启用 Claude Code 的终端配置？' : 'Use Claude Code&apos;s terminal setup?'}</Text>
          <Box flexDirection="column" width={70} gap={1}>
            <Text>
              {useChineseUi ? '为了获得更好的编码体验，请启用推荐设置' : 'For the optimal coding experience, enable the recommended settings'}
              <Newline />
              {useChineseUi ? '适用于你的终端：' : 'for your terminal:'}{' '}
              {env.terminal === 'Apple_Terminal' ? useChineseUi ? 'Option+Enter 输入换行并启用可视响铃' : 'Option+Enter for newlines and visual bell' : useChineseUi ? 'Shift+Enter 输入换行' : 'Shift+Enter for newlines'}
            </Text>
            <Select options={[{
            label: useChineseUi ? '是，使用推荐设置' : 'Yes, use recommended settings',
            value: 'install'
          }, {
            label: useChineseUi ? '否，稍后通过 /terminal-setup 设置' : 'No, maybe later with /terminal-setup',
            value: 'no'
          }]} onChange={value => {
            if (value === 'install') {
            ...
            <Text dimColor>
              {exitState.pending ? <>{useChineseUi ? '再次按 ' : 'Press '}{exitState.keyName}{useChineseUi ? ' 退出' : ' again to exit'}</> : <>{useChineseUi ? '回车确认 · Esc 跳过' : 'Enter to confirm · Esc to skip'}</>}
            </Text>
          </Box>
        </Box>
    });
  ...
  return <Box flexDirection="column">
      ...
      <Box flexDirection="column" marginTop={1}>
        ...
        {exitState.pending && <Box padding={1}>
            <Text dimColor>{useChineseUi ? '再次按 ' : 'Press '}{exitState.keyName}{useChineseUi ? ' 退出' : ' again to exit'}</Text>
          </Box>}
      </Box>
    </Box>;
}
