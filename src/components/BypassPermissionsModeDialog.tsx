...
export function BypassPermissionsModeDialog(t0) {
  ...
  if ($[3] === Symbol.for("react.memo_cache_sentinel")) {
    t3 = <Box flexDirection="column" gap={1}><Text>{useChineseUi ? '在 Bypass Permissions 模式下，Claude Code 执行潜在危险命令前不会再向你请求批准。' : 'In Bypass Permissions mode, Claude Code will not ask for your approval before running potentially dangerous commands.'}<Newline />{useChineseUi ? '此模式仅应在受限网络、可快速恢复的沙箱容器/虚拟机中使用。' : 'This mode should only be used in a sandboxed container/VM that has restricted internet access and can easily be restored if damaged.'}</Text><Text>{useChineseUi ? '继续即表示你接受在 Bypass Permissions 模式下产生的一切操作责任。' : 'By proceeding, you accept all responsibility for actions taken while running in Bypass Permissions mode.'}</Text><Link url="https://code.claude.com/docs/en/security" /></Box>;
    $[3] = t3;
  } else {
    t3 = $[3];
  }
  ...
  if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = [{
      label: useChineseUi ? '否，退出' : 'No, exit',
      value: "decline"
    }, {
      label: useChineseUi ? '是，我接受' : 'Yes, I accept',
      value: "accept"
    }];
    $[4] = t4;
  } else {
  ...
  let t5;
  if ($[5] !== onChange) {
    t5 = <Dialog title={useChineseUi ? '警告：Claude Code 正在以 Bypass Permissions 模式运行' : 'WARNING: Claude Code running in Bypass Permissions mode'} color="error" onCancel={handleEscape}>{t3}<Select options={t4} onChange={value_0 => onChange(value_0 as 'accept' | 'decline')} /></Dialog>;
    $[5] = onChange;
    $[6] = t5;
  } else {
    t5 = $[6];
