...
export function PowerShellPermissionRequest(props: PermissionRequestProps): React.ReactNode {
  ...
  return <PermissionDialog workerBadge={workerBadge} title={useChineseUi ? 'PowerShell 命令' : 'PowerShell command'}>
      ...
      {showPermissionDebug ? <>
          ...
          {toolUseContext.options.debug && <Box justifyContent="flex-end" marginTop={1}>
              <Text dimColor>{useChineseUi ? 'Ctrl-D 隐藏调试信息' : 'Ctrl-D to hide debug info'}</Text>
            </Box>}
        </> : <>
          <Box flexDirection="column">
            ...
            <Text>{useChineseUi ? '要继续吗？' : 'Do you want to proceed?'}</Text>
          ...
          <Box justifyContent="space-between" marginTop={1}>
            <Text dimColor>
              {useChineseUi ? 'Esc 取消' : 'Esc to cancel'}
              {(focusedOption === 'yes' && !yesInputMode || focusedOption === 'no' && !noInputMode) && (useChineseUi ? ' · Tab 修改' : ' · Tab to amend')}
              {explainerState.enabled && ` · ${useChineseUi ? 'ctrl+e ' : 'ctrl+e to '}${explainerState.visible ? useChineseUi ? '收起解释' : 'hide' : useChineseUi ? '查看解释' : 'explain'}`}
            </Text>
            {toolUseContext.options.debug && <Text dimColor>{useChineseUi ? 'Ctrl+D 显示调试信息' : 'Ctrl+d to show debug info'}</Text>}
          </Box>
        </>}
    </PermissionDialog>;
}
