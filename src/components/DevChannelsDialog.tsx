...
export function DevChannelsDialog(t0) {
  ...
  if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
    t2 = <Text>{useChineseUi ? '--dangerously-load-development-channels 仅用于本地 channel 开发。不要用这个选项运行你从互联网上下载的 channel。' : '--dangerously-load-development-channels is for local channel development only. Do not use this option to run channels you have downloaded off the internet.'}</Text>;
    t3 = <Text>{useChineseUi ? '请使用 --channels 来运行已批准的 channel 列表。' : 'Please use --channels to run a list of approved channels.'}</Text>;
    $[2] = t2;
    $[3] = t3;
  } else {
    t2 = $[2];
  ...
  let t5;
  if ($[6] !== t4) {
    t5 = <Box flexDirection="column" gap={1}>{t2}{t3}<Text dimColor={true}>{useChineseUi ? 'Channels（频道）: ' : 'Channels: '}{t4}</Text></Box>;
    $[6] = t4;
    $[7] = t5;
  } else {
    t5 = $[7];
  ...
  if ($[8] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = [{
      label: useChineseUi ? '我确认这是用于本地开发' : 'I am using this for local development',
      value: "accept"
    }, {
      label: useChineseUi ? '退出' : 'Exit',
      value: "exit"
    }];
    $[8] = t6;
  } else {
  ...
  let t8;
  if ($[11] !== t5 || $[12] !== t7) {
    t8 = <Dialog title={useChineseUi ? '警告：正在加载开发 channel' : 'WARNING: Loading development channels'} color="error" onCancel={handleEscape}>{t5}{t7}</Dialog>;
    $[11] = t5;
    $[12] = t7;
    $[13] = t8;
  } else {
