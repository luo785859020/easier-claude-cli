...
export function ClaudeMdExternalIncludesDialog(t0) {
  ...
  if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = <Text>{useChineseUi ? '这个项目的 CLAUDE.md 导入了当前工作目录之外的文件。对于第三方仓库，请不要允许。' : "This project's CLAUDE.md imports files outside the current working directory. Never allow this for third-party repositories."}</Text>;
    $[5] = t6;
  } else {
    t6 = $[5];
  }
  ...
  if ($[6] !== externalIncludes) {
    t7 = externalIncludes && externalIncludes.length > 0 && <Box flexDirection="column"><Text dimColor={true}>{useChineseUi ? '外部导入：' : 'External imports:'}</Text>{externalIncludes.map(_temp4)}</Box>;
    $[6] = externalIncludes;
    $[7] = t7;
  } else {
    t7 = $[7];
  ...
  if ($[8] === Symbol.for("react.memo_cache_sentinel")) {
    t8 = <Text dimColor={true}>{useChineseUi ? '重要：请只在你信任的文件上使用 Claude Code。访问不受信任文件可能带来安全风险。' : 'Important: Only use Claude Code with files you trust. Accessing untrusted files may pose security risks'}{" "}<Link url="https://code.claude.com/docs/en/security" />{" "}</Text>;
    $[8] = t8;
  } else {
    t8 = $[8];
  }
  ...
  if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
    t9 = [{
      label: useChineseUi ? '是，允许外部导入' : 'Yes, allow external imports',
      value: "yes"
    }, {
      label: useChineseUi ? '否，禁用外部导入' : 'No, disable external imports',
      value: "no"
    }];
    $[9] = t9;
  } else {
  ...
  if ($[12] !== handleEscape || $[13] !== t10 || $[14] !== t4 || $[15] !== t5 || $[16] !== t7) {
    t11 = <Dialog title={useChineseUi ? '允许导入外部 CLAUDE.md 文件吗？' : 'Allow external CLAUDE.md file imports?'} color="warning" onCancel={handleEscape} hideBorder={t4} hideInputGuide={t5}>{t6}{t7}{t8}{t10}</Dialog>;
    $[12] = handleEscape;
    $[13] = t10;
    $[14] = t4;
    $[15] = t5;
