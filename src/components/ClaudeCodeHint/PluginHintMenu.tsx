...
export function PluginHintMenu({
  ...
  const options = [{
    label: <Text>{useChineseUi ? '是，安装 ' : 'Yes, install '}<Text bold>{pluginName}</Text></Text>,
    value: 'yes'
  }, {
    label: useChineseUi ? '否' : 'No',
    value: 'no'
  }, {
    label: useChineseUi ? '否，不再显示插件安装提示' : "No, and don't show plugin installation hints again",
    value: 'disable'
  }];
  return <PermissionDialog title={useChineseUi ? '插件推荐' : 'Plugin Recommendation'}>
      <Box flexDirection="column" paddingX={2} paddingY={1}>
        <Box marginBottom={1}>
          <Text dimColor>
            {useChineseUi ? '命令 ' : 'The '}<Text bold>{sourceCommand}</Text>{useChineseUi ? ' 建议安装一个插件。' : ' command suggests installing a plugin.'}
          </Text>
        </Box>
        <Box>
          <Text dimColor>{useChineseUi ? '插件：' : 'Plugin:'}</Text>
          <Text> {pluginName}</Text>
        </Box>
        <Box>
          <Text dimColor>{useChineseUi ? '市场：' : 'Marketplace:'}</Text>
        ...
        <Box marginTop={1}>
          <Text>{useChineseUi ? '要安装吗？' : 'Would you like to install it?'}</Text>
        </Box>
        <Box>
